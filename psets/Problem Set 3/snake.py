...
class SnakeEnv(Environment[SnakeObservation, Direction]):
    ...
    def reset(self, seed: Optional[int] = None) -> Point:
        ...
        # Set the snake to the center of the grid
        self.snake = [Point(self.width // 2, self.height // 2)]
        self.direction = Direction.LEFT # The snake starts moving LEFT
    ...
    def actions(self) -> List[Direction]:
        ...
        """
        dir = self.direction
        return [Direction.NONE, Direction((dir + 1) % 4), Direction((dir - 1) % 4)]
    ...
    def step(self, action: Direction) -> \
        ...
        # Move the snake
        head = self.snake[0] + self.direction.to_vector()
        head = Point(head.x % self.width, head.y % self.height)  # Wrap around
        ...
        if head in self.snake:
            ...
            # Head is on the apple
            self.snake = [head] + self.snake  # Grow the snake
            reward += 1
            if len(self.snake) == self.width * self.height:
                ...
                self.apple = None
                reward += 100
            else: # Generate a new apple as the game is not over yet
                self.apple = self.generate_random_apple()
        else:
            # Normal move
            self.snake = [head] + self.snake[:-1] # Remove tail as snake didnt grow
